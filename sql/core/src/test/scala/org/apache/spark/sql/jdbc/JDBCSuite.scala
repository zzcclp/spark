...
class JDBCSuite extends SparkFunSuite
  ...
  test("SELECT * WHERE (simple predicates)") {
    ...
    assert(sql("SELECT * FROM foobar WHERE NAME != 'fred'").collect().size === 2)
    assert(sql("SELECT * FROM foobar WHERE NAME IN ('mary', 'fred')")
      .collect().size == 2)
    assert(sql("SELECT * FROM foobar WHERE NAME NOT IN ('fred')")
      .collect().size === 2)
    assert(sql("SELECT * FROM foobar WHERE THEID = 1 OR NAME = 'mary'")
      .collect().size == 2)
    assert(sql("SELECT * FROM foobar WHERE THEID = 1 OR NAME = 'mary' "
      + "AND THEID = 2").collect().size == 2)
    assert(sql("SELECT * FROM foobar WHERE NAME LIKE 'fr%'").collect().size == 1)
    assert(sql("SELECT * FROM foobar WHERE NAME LIKE '%ed'").collect().size == 1)
    assert(sql("SELECT * FROM foobar WHERE NAME LIKE '%re%'").collect().size == 1)
