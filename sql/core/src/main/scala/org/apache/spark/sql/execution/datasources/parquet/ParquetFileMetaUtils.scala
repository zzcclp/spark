...
object ParquetFileMetaUtils {
  ...
  @throws[IOException]
  def readFooterByRange(metaCacheEnabled: Boolean, useOldApiReadFooter: Boolean,
                        configuration: Configuration, file: Path,
                        start: Long, end: Long): ParquetMetadata = {
    if (metaCacheEnabled) {
      ...
      if (useOldApiReadFooter) {
        ParquetFileReader.readFooter(configuration, file, range(start, end))
      } else {
  ...
  @throws[IOException]
  def readFooterByNoFilter(metaCacheEnabled: Boolean, useOldApiReadFooter: Boolean,
                           configuration: Configuration, file: Path): ParquetMetadata = {
    if (metaCacheEnabled) {
      ParquetFileMeta.readFooterFromCache(file, configuration)
    }
    else {
      if (useOldApiReadFooter) {
        ParquetFileReader.readFooter(configuration, file, NO_FILTER)
      } else {
