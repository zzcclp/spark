...
class LocalDataCacheManagerSuite extends SparkFunSuite {
  ...
  test("Verify soft affinity strategy when some executors added and some executors removed") {
    ...
    val cm: LocalDataCacheManager = new LocalDataCacheManager(new SoftAffinityStrategy())
    ...
    // call init method after all executors ready
    cm.init()

    // check if init success
    assert(cm.executors.size == 4)
    assert(cm.fixedIdForExecutor.size == 4)
    assert(cm.nodeExecutorMap.size == 3)

    // case1 : all candidates alive
    val softAffinityCandidates1 = cm.askExecutor(partition1)
    ...
    assert(cm.fixedIdForExecutor.count(x => x.isDefined) == 3)
    val softAffinityCandidates2 = cm.askExecutor(partition1)
    ...
    assert(cm.fixedIdForExecutor.count(x => x.isDefined) == 2)
    val softAffinityCandidates3 = cm.askExecutor(partition1)
    ...
    assert(cm.fixedIdForExecutor.count(x => x.isDefined) == 3)
    val softAffinityCandidates4 = cm.askExecutor(partition1)
    ...
    assert(cm.fixedIdForExecutor.count(x => x.isDefined) == 4)
    val softAffinityCandidates5 = cm.askExecutor(partition1)
    ...
    assert(cm.fixedIdForExecutor.count(x => x.isDefined) == 2)
    val softAffinityCandidates6 = cm.askExecutor(partition1)
